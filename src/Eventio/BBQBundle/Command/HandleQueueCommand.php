 ...
 */
class HandleQueueCommand extends ContainerAwareCommand {
    ...
    private $exitRequestPending = false;

    public function receiveSIGTERM() {
            $this->logInfo('Terminate signal received');
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        $this->output = $output;
        $this->verbosityLevel = $input->getOption('verbosity');
        ...
        while (true) {
            ...
            if ($this->exitRequestPending) { break; }
        
            $jobsInLoop = 0;
            foreach ($queuesToQuery as $queue) {
                if ($this->exitRequestPending) { break 2; }
                
                $this->verboseWrite(9,'Fetching job from ' . $queue->getId());
                $job = $queue->fetchJob($waitTime);
                if ($this->exitRequestPending) {
                    if ($job) { $queue->releaseJob($job); }
                    break 2;
                }
                ...
                if ($job) {
                    ...
                    if (false === $process->isSuccessful()) {
                        ...
                        $this->logError(sprintf("Job execution failed. Queue '%s', job payload '%s'.", $queue->getId(), $jobPayload));
                        $this->logError(sprintf("Error output: %s", $process->getErrorOutput()));
